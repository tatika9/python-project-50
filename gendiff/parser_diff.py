...
def get_diff(dict1, dict2):
    def iter(name, node1, node2):
        ...
        for key in keys:
            if key not in node1:
                children.append(mk_plain(key, 'added', {'old': None, 'new': node2[key]}))
            elif key not in node2:
                children.append(mk_plain(key, 'deleted', {'old': node1[key], 'new': None}))
                ...
                children.append(mk_plain(key, 'unchanged', {'old': node1[key], 'new': node2[key]}))
            else:
                children.append(mk_plain(key, 'changed', {'old': node1[key], 'new': node2[key]}))
